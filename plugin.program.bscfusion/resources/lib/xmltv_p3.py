...
def elem_to_programme(elem):
    ...
    for epnumnode in elem.findall('episode-num'):
        if not d.__contains__('episode-num'):
            d['episode-num'] = []
    ...
    for stnode in elem.findall('subtitles'):
        if not d.__contains__('subtitles'):
            d['subtitles'] = []
        std = {}
    ...
    for srnode in elem.findall('star-rating'):
        if not d.__contains__('star-rating'):
            d['star-rating'] = []
        srd = {}
...
class Writer:
    ...
    **All strings passed to this class must be Unicode, except for dictionary
    ...
    def set_zero_ormore(self, programme, element, p):
        ...
        """
        if programme.__contains__(element):
    ...
    def addProgramme(self, programme):
        ...
        for attr in ('start', 'channel'):
            if programme.__contains__(attr):
        ...
        # Episode-num
        if programme.__contains__('episode-num'):
        ...
        # Audio details
        if programme.__contains__('audio'):
            a = SubElement(p, 'audio')
            if programme['audio'].__contains__('stereo'):
                s = SubElement(a, 'stereo')
                self.settext(s, programme['audio']['stereo'], with_lang=False)
            if programme['audio'].__contains__('present'):
        ...
        # Previously shown
        if programme.__contains__('previously-shown'):
            ps = SubElement(p, 'previously-shown')
            for attr in ('start', 'channel'):
                if programme['previously-shown'].__contains__(attr):
    ...
    def addChannel(self, channel):
        ...
        # Icon
        if channel.__contains__('icon'):
            self.seticons(c, channel['icon'])

        # URL
        if channel.__contains__('url'):
